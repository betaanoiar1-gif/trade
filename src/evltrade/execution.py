...
import random
from .core import D, Fill, MarketTick, Order, OrderType, Side, ZERO
...
class LatencyModel:
    ...
    def total_ms(self)->int:
        ...
        return max(0,base+random.Random(self.seed).randint(-self.jitter_ms,self.jitter_ms)) if self.mode==LatencyMode.STOCHASTIC and self.jitter_ms else base
...
class L2Book:
    ...
    def consume(self,side:Side,qty:Decimal,limit_price:Decimal|None=None)->list[tuple[Decimal,Decimal]]:
        levels=sorted(self.asks,key=lambda x:x.price) if side==Side.BUY else sorted(self.bids,key=lambda x:x.price,reverse=True)
        remaining=qty; out=[]
        for lvl in levels:
            ...
            available=max(ZERO,lvl.quantity-lvl.queue_ahead)
            take=min(remaining,available)
            if take>ZERO:
                lvl.quantity-=take; remaining-=take; out.append((lvl.price,take))
        return out
...
class ExecutionEngine:
    ...
    def execute(self,order:Order,tick:MarketTick,book:L2Book|None=None)->list[Fill]:
        ...
        if intent.post_only and intent.order_type==OrderType.MARKET:return []
        limit=intent.limit_price if intent.order_type in (OrderType.LIMIT,OrderType.STOP_LIMIT) else None
        if book:
            levels=book.consume(intent.side,order.remaining,limit)
            if intent.time_in_force.value=="fok" and sum(q for _,q in levels)<order.remaining:return []
            return [Fill(order.id,tick.ts,px+(self.impact.impact(px,q) if intent.side==Side.BUY else -self.impact.impact(px,q)),q,"taker" if intent.order_type==OrderType.MARKET else "maker") for px,q in levels]
