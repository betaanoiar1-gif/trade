...
class ExperimentCoordinator:
    ...
    def submit(self,fn,*args,**kwargs)->str:
        job_id=self.deterministic_id({"fn":getattr(fn,"__name__","job"),"args":args,"kwargs":kwargs})
        if job_id in self.jobs:return job_id
        self.jobs[job_id]=JobState(job_id,"running",0.0,started_at=time.time()); fut=self.pool.submit(fn,*args,**kwargs); self.futures[job_id]=fut; fut.add_done_callback(lambda f:self._done(job_id,f)); return job_id
